...
Server-wide cooperative events for Paper 1.21.x.
...
Put `GlobalEventBar-1.0.0.jar` in `plugins/`, then **stop and start** the server. Do not use `/reload`.

## Features

- Six gameplay types: mining, mob kills, fishing, farming, woodcutting, block placing
- Custom events scored by administrators
...
Sunucu geneli kooperatif etkinlikler, ortak BossBar, UUID sıralaması ve yapılandırılabilir ödüller. Kaynak kod yayınlanmaz; resmi JAR ücretsizdir.
